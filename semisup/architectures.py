...
def lenet(inputs,
  ...
  with slim.arg_scope(
          ...
          activation_fn=tf.nn.relu,
          weights_regularizer=slim.l2_regularizer(l2_weight)):
    net = slim.conv2d(net, 32, [5, 5], #weights_initializer = tf.truncated_normal_initializer(stddev=0.1),
                      scope='conv1')
    net = slim.max_pool2d(net, [2, 2], scope='pool1')
    net = slim.conv2d(net, 64, [5, 5],
                      #weights_initializer = tf.truncated_normal_initializer(stddev=0.1),
                      scope='conv2')
    net = slim.max_pool2d(net, [2, 2], scope='pool2')  # 14

    net = slim.flatten(net, scope='flatten')
    emb = slim.fully_connected(net, emb_size,
                               weights_initializer = tf.truncated_normal_initializer(stddev=0.1), scope='fc1')
    # if is_training:
    emb = slim.dropout(emb, keep_prob=0.5, is_training=is_training)
  return emb