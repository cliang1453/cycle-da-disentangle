...
from tools import mnist3 as mnist_tools
...
def main(_):
  train_images, train_labels = svhn_tools.get_data('train')
  test_images, test_labels = svhn_tools.get_data('test')
  ...
  with graph.as_default():
    model_function = partial(
      semisup.architectures.svhn_model,
      # new_shape=svhn_tools.IMAGE_SHAPE,
      img_shape=svhn_tools.IMAGE_SHAPE,
      augmentation_function=None,
      batch_norm_decay=0.99,
      emb_size=128)
    model = semisup.SemisupModel(model_function, NUM_LABELS,
                                 svhn_tools.IMAGE_SHAPE)
