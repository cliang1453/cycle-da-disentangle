    ...
    with graph.as_default():
        with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks,
            ...
            if FLAGS.unsup_samples != 0:
                match_ab,_ = model.add_semisup_loss(t_sup_emb,
                                       t_unsup_emb,
                                       t_sup_labels,
                                       visit_weight=visit_weight,
                                       walker_weight=walker_weight)
                best_match = tf.argmax(match_ab, axis=1)

                t_unsup_images_resize = tf.cast(tf.image.resize_images(
                    t_unsup_images,
                    tf.constant(new_shape[:2]),
                    method=tf.image.ResizeMethod.BILINEAR),tf.uint8)
                print(match_ab.shape)
                print(t_unsup_images_resize.shape)
                print(t_sup_images.shape)
                best_match_images = tf.gather(t_unsup_images_resize, best_match)
                print(best_match_images.shape)
                vis = tf.concat([t_sup_images, best_match_images], axis=2)
                vis = tf.reshape(vis, [FLAGS.sup_per_batch, new_shape[0] * num_labels, new_shape[1]*2, 3])
                tf.summary.image('sim', vis, max_outputs=8)
